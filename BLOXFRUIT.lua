        ...
        embeds = {{
            ...
            fields = {{
                ...
                value = GetCurrentDateTime() .. " | " .. DispTime(os.time() - StartTime, true) ..
                    " after execution\nMain task: " .. (ScriptStorage.Task.MainTask or "n/a") .. " ( " ..
                    (ScriptStorage.Task["MainTask-d"] and DispTime(os.time() - ScriptStorage.Task["MainTask-d"], true) or
                        "n/a") .. " ) \nSub task: " .. (ScriptStorage.Task.SubTask or "n/a") .. " ( " ..
                    (ScriptStorage.Task["SubTask-d"] and DispTime(os.time() - ScriptStorage.Task["SubTask-d"], true) or
                        "n/a") .. " )"
            }, {
                name = "Traceback",
                value = (function()
...
function mmb()
    ...
    function BlurManager:SetIntensity(intensity)
        ...
        local tweenInfo = TweenInfo.new(0.3, -- Time
    ...
    function SetText(Name, Text)
        task.spawn(function()
            ...
            local tweenService = game:GetService("TweenService")
            local fadeOutInfo = TweenInfo.new(0.3, -- Time
            ...
            local fadeInInfo = TweenInfo.new(0.3, -- Time
            Enum.EasingStyle.Quad, -- Easing style
            Enum.EasingDirection.Out -- Easing direction
            )
    ...
    function ToggleUI(State)
        ...
        local tweenService = game:GetService("TweenService")
        local tweenInfo = TweenInfo.new(0.5, -- Time
        ...
        if isToggleOpen then
            ...
            local rotationTween = tweenService:Create(ToggleIcon, TweenInfo.new(0.5, Enum.EasingStyle.Back,
            ...
            shrinkTween.Completed:Connect(function()
                local growTween = tweenService:Create(ToggleIcon, TweenInfo.new(0.3, Enum.EasingStyle.Back,
                    Enum.EasingDirection.Out), {
    ...
    ToggleButton.MouseEnter:Connect(function()
        ...
        local pulseSequence = function()
            local expandTween = tweenService:Create(ToggleContainer, TweenInfo.new(0.5, Enum.EasingStyle.Quad,
                Enum.EasingDirection.Out), {
    ...
    local Segmants = {"RawConstants", "Utilly", "QuestManager", "SpawnRegionLoader", "TweenController",
                      "AttackController", "CombatController", "FunctionsHandler", "Hooks", "Debug", "Hop", "Storage"}
        ...
        if Child and typeof(Child) == "Instance" and Child:IsA("Tool") then
            ...
            elseif string.find(tostring(Child), "Fruit") then
                task.spawn(function()
                    ...
                    local StoreResult = Remotes.CommF_:InvokeServer("StoreFruit", Child:GetAttribute("OriginalName"),
                        Child)
    ...
    GodhumanMaterials = {
        ["Fish Tail"] = {20, 3, {"Fishman Raider", "Fishman Captain"},
                         {"DeepForestIsland3", 1, 1775, "Turtle Adventure Quest Giver"}},
        ["Dragon Scale"] = {10, 3, {"Dragon Crew Warrior", "Dragon Crew Archer"},
                            {"DragonCrewQuest", 1, 1575, "Dragon Crew Quest Giver"}},
        ["Magma Ore"] = {20, 2, {"Magma Ninja"}, {"FireSideQuest", 1, 1100, "Fire Quest Giver"}},
        ["Mystic Droplet"] = {10, 2, {"Sea Soldier", "Water Fighter"},
                              {"ForgottenQuest", 2, 1425, "Forgotten Quest Giver"}}
    }

    SeaIndexes = {"Main", "Dressrosa", "Zou"}

    TasksOrder = {"SpecialBossesTask", "RaidController", "Trevor", "UtillyItemsActivitation", "ColosseumPuzzle",
                  "Wenlocktoad", "ThirdSeaPuzzle", "PirateRaid", "SecondSeaPuzzle", "ThirdSeaPuzzle", "CollectDrops",
                  "BossesTask", "ExpRedeem", "LevelFarm"}
    ...
    Portals = ({{Vector3.new(-7894.6201171875, 5545.49169921875, -380.246346191406),
                ...
                {Vector3.new(-288.46246337890625, 306.130615234375, 597.9988403320312),
                 ...
                 Vector3.new(-6508.5581054688, 89.034996032715, -132.83953857422)}, {}})[SeaIndex]
    ...
    function CheckIsPlayerAlive(Instance)
        ...
        return
            Instance and Instance.Character and Instance.Character.Humanoid and Instance.Character.HumanoidRootPart and
                Instance.Character.Head and Instance.Character.Humanoid.Health > 0 -- nuh uh
    end
    ...
    function GetMonAsSortedRange()
        ...
        table.foreach(Services.Workspace.Enemies:GetChildren(), function(_, Mon)
            if Mon and Mon:FindFirstChild("Humanoid") and Mon:FindFirstChild("HumanoidRootPart") and Mon.Humanoid.Health >
                0 then
    ...
    function QuestManager.RefreshQuest(Self)
        ...
        for QuestID, QuestDatas in QuestManager.Quests do
            if not QuestManager.BlacklistedQuestIds[QuestID] then
                ...
                if (QuestDatas[1].LevelReq >= QuestLevelFlag and QuestDatas[1].LevelReq <=
                    ScriptStorage.PlayerData.Level) then
    ...
    function QuestManager.GetCurrentQuest(Self)
        ...
        local QuestIndex = Self.CurrentQuests[Self.CurrentLevel] and Self.CurrentQuests[Self.CurrentLevel].LevelReq <=
                               ScriptStorage.PlayerData.Level and Self.CurrentLevel or 1
    ...
    function TweenController.Tween2(ePart, Position)
        TweenInstance2 = Services.TweenService:Create(ePart, TweenInfo.new(
            CaculateDistance(ePart.CFrame, Position) / 50, Enum.EasingStyle.Linear), {
    ...
    function TweenController.Create(Position)
        ...
        if not Position or TweenDebounce then
            return
        end
    ...
    function FastAttack:GetBladeHits(Character, Distance)
        ...
        if AttackConfig.AttackMobs then
            pcall(ProcessTargets, Workspace.Enemies)
        end
    ...
    function Getplayerhit()
        ...
        for _, v in pairs(workspace.Characters:GetChildren()) do
            if v.Name ~= game.Players.LocalPlayer.Name and v:FindFirstChild("Humanoid") and
            ...
            if Mon.Name == MobName then
                ...
                Mon:FindFirstChild("Humanoid") and Mon.Humanoid.Health > 0 then
                    ...
                    if MonPosition and isnetworkowner(Mon.PrimaryPart) then
                        if not ForcePosition or CaculateDistance(MonPosition, ForcePosition) <
                            CombatController.GRAB_DISTANCE then
        ...
        for _, Child in (MobTable) do
            ...
            if MonResult then
                ...
                while task.wait() do
                    ...
                    if ScriptStorage.Tools["Sweet Chalice"] and
                    ...
                    if CaculateDistance(MobHumanoidRootPart.Position + Vector3.new(0, 35, 0)) < 150 then
                        ...
                        if MonResult.Name ~= "Core" then
                            if ScriptStorage.PlayerData.Level > 100 and Count2 >= CombatController.MAX_ATTACK_DURATION_2 and
                                MobHumanoid.Health - MobHumanoid.MaxHealth == 0 then
                                SetTask("SubTask",
                                    "Hop Server - Mob Health Unchanged ( " .. MobHumanoid.Health .. " / " ..
                                        MobHumanoid.MaxHealth .. ")")
                                alert("Stuck", "Mob health unchanged")
                                _G.Stop = true
                                Hop("Mob Health Stuck")
                                game.Players.LocalPlayer:Kick("Fix Stuck")
                            end

                            if (MonResult:GetAttribute("FailureCount") or 0) > 5 then
                                Hop("Failed to attack")
                            end
                            if Count >= CombatController.MAX_ATTACK_DURATION and MobHumanoid.Health -
                                ...
                                if OldPosition then
                                    ...
                                    alert("Failed to attack", "Returning to the old position ( #" ..
                                        MonResult:GetAttribute("FailureCount") .. " )")
                                    while CaculateDistance(MonResult.HumanoidRootPart.CFrame, OldPosition) > 6 and
                                        task.wait() do
    ...
    FunctionsHandler.ExpRedeem:RegisterMethod("Start", function()
        local Code = ({"BANEXPLOIT", "NOMOREHACK", "WildDares", "BossBuild", "GetPranked", "EARN_FRUITS",
                       ...
                       "Sub2OfficialNoobie", "TheGreatAce", "SEATROLLING", "24NOADMIN", "ADMIN_TROLL", "NEWTROLL",
    ...
    FunctionsHandler.LevelFarm:RegisterMethod("Start", function(Level)
        if SeaIndex == 3 then
            if (ScriptStorage.Backpack.Bones or {
                Count = 0
            }).Count >= 50 and (ScriptStorage.PlayerData.Level < MaxLevel or ForceToRollBone) then
        ...
        if GodHumanFlag then
            ...
            local Material, MaterialData = (function()
                ...
                for Material, MaterialData in GodhumanMaterials do
                    if (ScriptStorage.Backpack[Material] or {
                        Count = 0
                    }).Count < MaterialData[1] then
            ...
            if Material then
                ...
                if PlayerLevel >= MaterialData[4][3] then
                    ...
                    if QuestAvailable then
                        if not string.find(CurrentClaimQuest, MaterialData[3][1]) and
                            not string.find(CurrentClaimQuest, MaterialData[3][2]) then
        ...
        if ScriptStorage.Tools["God's Chalice"] and not ScriptStorage.Tools["Mirror Fractal"] then
            if (ScriptStorage.Backpack["Conjured Cocoa"] or {
                Count = 0
            }).Count < 10 then
        ...
        if ScriptStorage.Tools["Sweet Chalice"] or (PlayerLevel == MaxLevel and (ScriptStorage.Backpack.Bones or {
            Count = 0
        }).Count >= 500) then
        ...
        if PlayerLevel >= 2025 and
            (getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost == 0 or PlayerLevel == MaxLevel) and
            (ScriptStorage.Backpack.Bones or {
                Count = 0
            }).Count < 500 then
    ...
    FunctionsHandler.Saber:RegisterMethod("Start", function()
        local Progress, LastestRefreshSenque = FunctionsHandler.Saber:Get("CurrentProgressLevel"),
    ...
    FunctionsHandler.MeleesController:RegisterMethod("Start", function()
        for Cursor, Melee in MeleesTable do
            if Melee ~= 'SanguineArt' then
                ...
                if not ScriptStorage.Melees[Melee] or (ScriptStorage.Melees[Melee] or 0) < Data.NextLevelRequirement then
                    ...
                    if not MSet and ScriptStorage.Melees[Melee] and ScriptStorage.Melees[Melee] <
                        Data.NextLevelRequirement then
                        SetTask("SubTask",
                            "Farming Until Enough Mastery For " .. Melee .. " ( " .. ScriptStorage.Melees[Melee] ..
        ...
        if FarmingItem then
            SetTask('SubTask', 'Farming mastery for ' .. FarmingItem[1] .. ' ( ' .. FarmingItem[2] .. ' / ' ..
                FarmingItem[3] .. ' )')
    ...
    FunctionsHandler.SecondSeaPuzzle:RegisterMethod("Start", function()
        local Progress, LastestRefreshSenque = FunctionsHandler.SecondSeaPuzzle:Get("CurrentProgressLevel"),
    ...
    FunctionsHandler.ColosseumPuzzle:RegisterMethod("Start", function()
        local Progress, LastestRefreshSenque = FunctionsHandler.ColosseumPuzzle:Get("CurrentProgressLevel"),
        ...
        elseif Progress == 3 then
            ...
            if CaculateDistance(CFrame.new(-1837.46155, 44.2921753, 1656.1987, 0.999881566, -1.03885048e-22,
                -0.0153914848, 1.07805858e-22, 1, 2.53909284e-22, 0.0153914848, -2.55538502e-22, 0.999881566)) > 10 then
                alert("tween to")
    ...
    FunctionsHandler.EvoRace:RegisterMethod("Refresh", function()
        ...
        if getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost ~= 0 or ScriptStorage.PlayerData.Level <
            900 or ScriptStorage.PlayerData.Beli < 1000000 or ScriptStorage.PlayerData.RaceLevel ~= 1 then
            return
        end
        return true
    end)
    ...
    FunctionsHandler.BossesTask:RegisterMethod("Refresh", function()
        ...
        if Boss and
            (CaculateDistance(Boss.HumanoidRootPart.CFrame) < (SeaIndex == 2 and 3000 or 5000) or
                BossesOrderWL[tostring(Boss)] or ScriptStorage.PlayerData.Level == MaxLevel) then
            return Boss
        end
    end)
    ...
    FunctionsHandler.BossesTask:RegisterMethod("Start", function(Boss)
        ...
        if Boss then
            ...
            CombatController.Attack(tostring(Boss), null, null, function()
                SpecialItems = nil
            end)
    ...
    FunctionsHandler.RaidController:RegisterMethod("Refresh", function()
        ...
        return RaidFruit or FunctionsHandler.RaidController.Methods.GetCurrentRaidIsland:Call() or
                   CheckSpecialMicrochip()
    end)
    ...
    FunctionsHandler.RaidController:RegisterMethod("Start", function()
        ...
        if not CurrentIsland then
            ...
            if not workspace.Map:FindFirstChild(RootRaidIsland) and not workspace:FindFirstChild(RootRaidIsland) then
                task.wait(1)
                return TweenController.Create(game:GetService("ReplicatedStorage").FakeIslands:FindFirstChild(
                    RootRaidIsland):GetModelCFrame())
            end
            ...
            fireclickdetector(
                (workspace.Map:FindFirstChild(RootRaidIsland) or workspace:FindFirstChild(RootRaidIsland)).RaidSummon2
                    .Button.Main.ClickDetector)
            local RaidStartSenque = os.time()
            SetTask("MainTask", "Auto Raid - Waiting Until Raid Is Started")
            repeat
                task.wait()
            until os.time() - (LastRaidAlert2 or 0) < 20 or os.time() - RaidStartSenque > 30
            TweenController.Create(LocalPlayer.Character.HumanoidRootPart.CFrame)
            repeat
                task.wait()
            until os.time() - (LastRaidAlert or 0) < 20 or os.time() - RaidStartSenque > 30
            ...
            for _, Mon in GetMonAsSortedRange() do
                ...
                while Mon and Mon:FindFirstChild("HumanoidRootPart") and Mon.Humanoid.Health > 0 and
                    CaculateDistance(Mon.HumanoidRootPart.Position) < 1000 and os.time() - StartTick1 < 60 and
                    task.wait(.05) do
                    Found = true
    ...
    FunctionsHandler.CollectDrops:RegisterMethod('Refresh', function()
        ...
        for _, Fruit in workspace:GetChildren() do
            if string.find(Fruit.Name, "Fruit") and not Players:FindFirstChild(Fruit.Name) and
                Fruit:FindFirstChild("Handle") and not FruitNames[tostring(Fruit)] and
                not ScriptStorage.Backpack[FruitNameToId(tostring(Fruit))] then
    ...
    FunctionsHandler.UtillyItemsActivitation:RegisterMethod("Refresh", function()
        ...
        if not SpecialItems then
            ...
            print(5)
            local Response = not ScriptStorage.Melees["Sharkman Karate"] and
        ...
        print(12)
        if SeaIndex == 3 and (ScriptStorage.Melees["Death Step"] or 0) >= 400 and
            (ScriptStorage.Melees["Black Leg"] or 0) >= 400 and ScriptStorage.PlayerData.Beli >= 2500000 and
            ScriptStorage.PlayerData.Fragments >= 5000 and not ScriptStorage.Melees["Electric Claw"] then
    ...
    FunctionsHandler.PirateRaid:RegisterMethod("Start", function()
        ...
        if NearestMon[1] then
            local MonHumanoid, MonHumanoidRootPart = NearestMon[1]:FindFirstChild("Humanoid"),
                NearestMon[1]:FindFirstChild("HumanoidRootPart")

            if MonHumanoidRootPart and MonHumanoid and MonHumanoid.Health > 0 and
                CaculateDistance(MonHumanoidRootPart.CFrame, SeaCastlePosition) < 500 then
    ...
    FunctionsHandler.SoulGuitar:RegisterMethod("Refresh", function()
        ...
        local EctoplasmCount = (ScriptStorage.Backpack["Ectoplasm"] or {
            Count = 0
        })["Count"]
        local BonesCount = (ScriptStorage.Backpack["Bones"] or {
            Count = 0
        })["Count"]
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("Refresh", function()
        ...
        if Backpack["Cursed Dual Katana"] or not Backpack.Tushita or Backpack.Tushita.Mastery < 350 or not Backpack.Yama or
            Backpack.Yama.Mastery < 350 then
            return
        end
        ...
        if CdkProgess.Opened then
            for Index, Value in CdkProgess do
                if Index ~= "Opened" and Index ~= "Finished" and Value < 3 then
                    print(Index, Value)
                    ScriptStorage.CdkCache = {Index, Value + 1}

                    if not ScriptStorage.Tools[ScrollSides[Index]] then
                        Remotes.CommF_:InvokeServer("LoadItem", ScrollSides[Index])
                    end

                    alert("Cursed Dual Katana", "Start " .. tostring(ScrollSides[Index]) .. " " .. tostring(Index))
                    Remotes.CommF_:InvokeServer("CDKQuest", "StartTrial", Index)
                    SetTask("MainTask",
                        "Cursed Dual Katana - " .. tostring(ScrollSides[Index]) .. " " .. tostring(Index))
                    return false
                end
            end
        end
    ...
    FunctionsHandler.CursedDualKatana:RegisterMethod("Start", function(CachedData)
        ...
        elseif CachedData[1] == "Evil" then
            if CachedData[2] == 1 then
                ...
                TweenController.Create((Mob and Mob.HumanoidRootPart.CFrame) or
                                           ScriptStorage.MobRegions["Forest Pirate"][0])
                CdkProgess = nil
    ...
    LowHop = function(Reason, PlayerLimit)
        local servers = {}
        local req = game:HttpGet("https://games.roblox.com/v1/games/" .. game.PlaceId ..
                                     "/servers/Public?sortOrder=Asc&limit=100&excludeFullGames=true")
        local body = game:GetService('HttpService'):JSONDecode(req)

        if body and body.data then
            for i, v in next, body.data do
                if type(v) == "table" and tonumber(v.playing) and tonumber(v.maxPlayers) and v.playing < 5 and v.id ~=
                    JobId then
    ...
    for _, Connection in getconnections(game.Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Main")
                                            :WaitForChild("Settings"):WaitForChild("Buttons")
                                            :WaitForChild("FastModeButton").Activated) do
        Connection.Function()
    end
        ...
        while task.wait() do
            if not _G.Stop then
                ...
                if true or RefreshDebounce ~= os.time() then
                    ...
                    if ScriptStorage.Interface then
                        SetText("LiveTime",
                            "Total Elapsed Time: " .. DispTime(TotalElapsed, true) .. " Elapsed Time: " ..

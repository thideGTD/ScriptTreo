...
local function WorldLoad()
    while true do
        ...
        if a.GetData().GamePasses.gp_gamespeed_3 then
            ...
            if LocalPlayer.PlayerGui.GameGuiNoInset.Screen.Top.WaveControls.TickSpeed.Items["2"].ImageColor3 ~= Color3.fromRGB(115, 230, 0) and LocalPlayer.PlayerGui.GameGuiNoInset.Screen.Top.WaveControls.TickSpeed.Items["3"].ImageColor3 ~= Color3.fromRGB(115, 230, 0) then
                local args = {
                    2
                }
                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("ChangeTickSpeed"):InvokeServer(unpack(args))
...
local function PlayV2()
    ...
    local AllPositions = {
        ...
        {113.36101531982422 + math.random(-1 ,1), 2.244499921798706 + math.random(-1 ,1), 112.03187561035156 + math.random(-1 ,1)}, -- Vị trí 1
        {-95.7408447265625 + math.random(-1 ,1), 2.24399995803833 + math.random(-1 ,1), 89.54883575439453 + math.random(-1 ,1)},  -- Vị trí 4
        {-76.21514892578125 + math.random(-1 ,1), 2.24399995803833 + math.random(-1 ,1), -120.4250259399414 + math.random(-1 ,1)} -- Vị trí 3
    }
